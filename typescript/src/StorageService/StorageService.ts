...
class StorageService {
  ...
  public deleteEventById(targetId: string) {
    localStorage.setItem(
      ...
      JSON.stringify(
        Object.fromEntries(
          Object.entries(this._getEvents()).map(([key, itsEvents]: [string, any[]]) => {
  ...
  public getMonthOfYear() {
    ...
    return `${MONTHS[referenceSunday.getMonth()]} ${referenceSunday.getFullYear()}`;
  }
  ...
  public setSelectedWeek(date: Date, index: number) {
    const cachedWeeks = JSON.parse(sessionStorage.getItem("cachedWeeks") || "[]").map(
      (dateTimeString: string) => new Date(dateTimeString)
    );
