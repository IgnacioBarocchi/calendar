...
class NavigationControls {
    ...
    _getDateOf(requestedWeekView) {
        const numberOfDaysToAdd = { ongoing: 0, next: 7, prev: -7 }[requestedWeekView];
        const today = new Date();
        if (numberOfDaysToAdd === 0)
            return today;
        return new Date(today.setDate(today.getDate() + numberOfDaysToAdd * this.navigationIndex));
    }
    // !super ugly
    _handleNavigationChange(requestedWeekView) {
        ...
        this.storage.setSelectedWeek(this._getDateOf(requestedWeekView), this.navigationIndex);
        this.reRenderControlledComponents();
    }
    reRenderControlledComponents() {
        this.calendarMonth.calendarBodyElement.innerHTML = "";
        [this.header, this.calendarBody, this.calendarMonth].forEach((component) => {
            component.render();
        });
    }
    ...
    navigateWithNavbar() {
        this.stages.forEach((requestedWeekView) => {
            ...
            activeButton.addEventListener("click", () => this._handleNavigationChange.bind(this)(requestedWeekView));
        });
    }
}
